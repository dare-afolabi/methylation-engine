...
def test_get_config_returns_singleton():
    ...
    assert (
        g1.get_platform("EPIC")["cost_per_sample"]
        == DEFAULT_PLATFORMS["EPIC"]["cost_per_sample"]
    )
...
def test_load_from_excel_and_export_excel_roundtrip(tmp_path):
    ...
    platforms_df = pd.DataFrame(
        [{"platform_id": "XPLAT", "name": "XPlatform", "n_cpgs": 123}]
    )
    designs_df = pd.DataFrame(
        [{"design_id": "XDES", "name": "XDesign", "description": "desc"}]
    )
    costs_df = pd.DataFrame(
        [{"component_id": "XCOMP", "cost": 77, "unit": "per_sample"}]
    )
    timeline_df = pd.DataFrame(
        [{"phase_id": "XPH", "name": "XPhase", "base_duration_days": 2}]
    )

    excel_path = tmp_path / "xlsx"
...
def test_get_platform_default_when_missing():
    ...
    assert result["name"] == DEFAULT_PLATFORMS["EPIC"]["name"]
...
def test_get_design_default_when_missing():
    ...
    assert isinstance(result, dict)
    assert result["name"] == DEFAULT_DESIGNS["two_group"]["name"]
...
def test_get_cost_components_filters_optional_and_applies_to():
    ...
    # if filtering by platform not in applies_to, bisulfite_conversion shouldn't be used
...
def test_update_regional_pricing_multiplies_and_int_casts():
    ...
    update_regional_pricing(region="TEST", multiplier=1.3)
