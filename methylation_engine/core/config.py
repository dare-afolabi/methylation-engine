...
class PlannerConfig:
    ...
    def export_to_excel(self, filepath: str):
        ...
        with pd.ExcelWriter(filepath, engine="openpyxl") as writer:
            # Platforms
            pd.DataFrame(self.platforms).T.to_excel(writer, sheet_name="Platforms")

            # Designs
            pd.DataFrame(self.designs).T.to_excel(writer, sheet_name="Designs")

            # Costs
            pd.DataFrame(self.cost_components).T.to_excel(writer, sheet_name="Costs")

            # Timeline
            pd.DataFrame(self.timeline_phases).T.to_excel(writer, sheet_name="Timeline")
    ...
    def list_platforms(self, recommended_only: bool = False) -> pd.DataFrame:
        ...
        if recommended_only:
            platforms = {
                k: v for k, v in platforms.items() if v.get("recommended", False)
            }
    ...
    def list_designs(self) -> pd.DataFrame:
        ...
        columns = ["name", "description", "complexity", "min_n_recommended", "paired"]
...
def update_regional_pricing(region: str = "US", multiplier: float = 1.0):
    ...
    for platform_id in config.platforms:
        cost_raw = config.platforms[platform_id].get("cost_per_sample", 0)
        current_cost = float(cast(Union[str, float, None], cost_raw) or 0)
        config.platforms[platform_id]["cost_per_sample"] = int(
            current_cost * multiplier
        )

    for comp_id in config.cost_components:
        cost_raw = config.cost_components[comp_id].get("cost", 0)
        current_cost = float(cast(Union[str, float, None], cost_raw) or 0)
...
def get_platform_by_budget(max_cost_per_sample: float) -> pd.DataFrame:
    ...
    platforms = {
        ...
        if float(cast(Union[str, float, None], v["cost_per_sample"]) or 0)
        <= max_cost_per_sample
    }
