...
df["Min ΔM"] = df["Min ΔM"].map(lambda x: f"{x:.2f}")
df["Min Δβ (approx)"] = df["Min Δβ (approx)"].map(lambda x: f"{x:.3f}")
...
plot_power_curves(
    effect_sizes=[delta_m * 0.5, delta_m, delta_m * 1.5, delta_m * 2],
...
plot_sample_size_heatmap(
    effect_sizes=[0.5, 1.0, 1.5, 2.0, 2.5],
    target_powers=[0.70, 0.80, 0.90],
    alpha=CONFIG["alpha"],
    paired=design_info['paired'],
    design_label=design_info['name'],
    save_path=f"{report_path}/sample_size_heatmap.png",
)

pdf.log_image(
    f"{report_path}/sample_size_heatmap.png",
    "Figure 2",
...
pdf.log_text(f"- `{report_path}/sample_size_heatmap.png`")
