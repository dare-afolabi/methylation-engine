...
from methylation_engine.core.config import get_config, PlannerConfig
...
def calculate_power(
    ...
    se = np.sqrt(variance / n_per_group) if paired else np.sqrt(2 * variance / n_per_group)
    ncp = effect_size / se
...
def sample_size_for_power(
    ...
    for n in range(3, max_n + 1):
        power = calculate_power(n, effect_size, alpha, variance, prior_df, paired=paired)
...
def detectable_effect_size(
    ...
    1.48
    """
    for effect in np.linspace(0.1, 5.0, 100):
        power = calculate_power(n_per_group, effect, alpha, variance, prior_df, paired=paired)
        if power >= target_power:
            return effect
...
def plan_sample_size(
    ...
    if expected_delta_beta is not None and expected_effect_size is None:
        ...
        raise ValueError("Must provide either expected_effect_size or expected_delta_beta")
    ...
    for level, n in options.items():
        power = calculate_power(n, expected_effect_size, alpha, variance, prior_df, paired=paired)
        det_effect = detectable_effect_size(n, target_power, alpha, variance, prior_df, paired=paired)
...
def estimate_costs(
    ...
    # Calculate costs
    costs = {
        'platform': platform_info['cost_per_sample'] * n_samples
    }
...
def estimate_timeline(
    ...
    include_optional_phases : bool
        Include validation and manuscript phases
    config : PlannerConfig, optional
        Configuration instance (uses global if None)
    ...
    for phase_id, phase_config in config.timeline_phases.items():
        ...
        # Calculate duration
        duration = phase_config['base_duration_days']
        
        # Apply scaling factor based on sample size
        if phase_config['scaling_factor'] > 0:
            duration += n_samples * phase_config['scaling_factor']
        
        # Apply batch adjustment for array processing
        if 'batch_adjustment' in phase_config:
            n_batches = int(np.ceil(n_samples / 96))
            duration += (n_batches - 1) * phase_config['batch_adjustment']
        
        # Adjust array processing for platform-specific processing days
        if phase_id == 'array_processing':
            duration = platform_info['processing_days'] + (n_samples // 96) * 2
...
def plan_batches(
    ...
    for i in range(n_batches):
        ...
        for g in range(n_groups):
            group_dist[f'group{g+1}'] = samples_per_group + (1 if g < (batch_size % n_groups) else 0)
        
        # Calculate balance ratio
        group_sizes = list(group_dist.values())
        balance = min(group_sizes) / max(group_sizes) if max(group_sizes) > 0 else 1.0
...
def assess_study_risks(
    ...
    if n_samples < 20:
        risks.append({
            ...
            'mitigation': 'Use paired design if possible, strict QC, validate results'
        })
    ...
    if budget is not None and estimated_cost is not None:
        if estimated_cost > budget:
            ...
            over_pct = deficit / budget
            if over_pct <= 0.10:
                severity = 'Medium'
                impact = 'Minor overrun; likely solvable with small scope tweaks'
                mitigation = 'Trim optional costs, negotiate rates, or reduce validation scope'
            elif over_pct <= 0.30:
                severity = 'High'
                impact = 'Material overrun; may require scope change or additional funding'
                mitigation = 'Reduce sample size, switch to cheaper platform, or secure funds'
            else:
                severity = 'High'
    ...
    if n_batches > 3:
        risks.append({
            ...
            'mitigation': 'Balance groups across batches, include batch in model, use ComBat'
        })
    ...
    if platform in ['WGBS', 'Nanopore']:
        risks.append({
            ...
            'mitigation': 'Ensure bioinformatics support available, allocate extra time'
        })
    ...
    if not platform_info.get('recommended', False):
        risks.append({
            ...
            'mitigation': 'Consider using a recommended platform like EPIC or EPICv2'
        })
...
def plot_power_curves(
    effect_sizes: List[float] = [0.5, 1.0, 1.5, 2.0],
    ...
    for effect in effect_sizes:
        powers = [calculate_power(n, effect, alpha, paired=paired) for n in sample_sizes]
        delta_beta = effect * 0.08
        ax.plot(
            sample_sizes, powers,
...
def plot_sample_size_heatmap(
    effect_sizes: List[float] = [0.5, 1.0, 1.5, 2.0, 2.5],
    target_powers: List[float] = [0.70, 0.80, 0.90],
    alpha: float = 0.05,
    save_path: Optional[str] = None,
    dpi: int = 300,
    paired: bool = False,
    ...
    """
    Create sample size recommendation heatmap.
    ...
    for power in target_powers:
        row = []
        for effect in effect_sizes:
            n = sample_size_for_power(power, effect, alpha, max_n=100, paired=paired)
            row.append(n)
        results.append(row)
    
    df = pd.DataFrame(
        results,
        index=[f'{p*100:.0f}% Power' for p in target_powers],
        columns=[f'ΔM={e:.1f}' for e in effect_sizes]
    )
    ...
    ax.set_xticks(np.arange(len(effect_sizes)))
    ax.set_yticks(np.arange(len(target_powers)))
    ax.set_xticklabels(df.columns)
    ax.set_yticklabels(df.index)
    
    for i in range(len(target_powers)):
        for j in range(len(effect_sizes)):
            ax.text(
                j, i, f'{df.values[i, j]:.0f}',
                ha="center", va="center",
    ...
    ax.set_xlabel('Expected Effect Size', fontsize=12)
    ax.set_ylabel('Target Power', fontsize=12)
    
    cbar = plt.colorbar(im, ax=ax)
    cbar.set_label('Sample Size', rotation=270, labelpad=20)
...
def create_study_plan(
    ...
    if output_dir is not None:
        ...
        plot_sample_size_heatmap(
            save_path=os.path.join(output_dir, 'sample_size_heatmap.png')
        )
...
def _generate_text_report(study_plan: Dict[str, Any], output_dir: str):
    ...
    with open(report_path, 'w') as f:
        ...
        f.write("=" * 80 + "\n")
        f.write(f"Generated: {study_plan['created_date'].strftime('%Y-%m-%d %H:%M:%S')}\n\n")
...
def _export_to_excel(study_plan: Dict[str, Any], output_dir: str):
    ...
    with pd.ExcelWriter(excel_path, engine='openpyxl') as writer:
        ...
        config_data = []
        cfg = study_plan['configuration']
        for key in ['expected_delta_beta', 'expected_effect_size', 
                    'target_power', 'alpha', 'design_type', 'platform']:
            if key in cfg:
        ...
        cost_data = pd.concat([
            cost_data,
            pd.DataFrame([{'Category': 'TOTAL', 'Cost': costs['total']}])
        ])
...
def quick_recommendation(
    ...
    rec = f"Recommend {n} per group ({n_total} total) for 80% power ({design_info['name']}). "
