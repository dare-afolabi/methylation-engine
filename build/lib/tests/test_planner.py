...
class TestEffectSizeConversion:
    ...
    def test_baseline_effects(self):
        """Different baselines give different ΔM values."""
        delta_beta = 0.10
        
        m_low = delta_beta_to_delta_m(delta_beta, baseline_beta=0.2)
        m_mid = delta_beta_to_delta_m(delta_beta, baseline_beta=0.5)
        m_high = delta_beta_to_delta_m(delta_beta, baseline_beta=0.8)
        
        # At baseline 0.2, 10% increase is larger on M scale
...
class TestSampleSizePlanning:
    ...
    def test_effect_size_conversion(self):
        ...
        assert abs(result1['recommended']['n_per_group'] - 
                   result2['recommended']['n_per_group']) <= 2
    ...
    def test_design_types(self, test_config):
        ...
        assert result_paired['recommended']['n_per_group'] < \
               result_two['recommended']['n_per_group']
    
    def test_power_levels(self):
        """Higher power requires more samples."""
        result_70 = plan_sample_size(
            expected_delta_beta=0.10,
            target_power=0.70
        )
        
        result_90 = plan_sample_size(
            expected_delta_beta=0.10,
            target_power=0.90
        )
        
        assert result_90['recommended']['n_per_group'] > \
...
class TestCostEstimation:
    ...
    def test_platform_differences(self, test_config):
        ...
        cost_450k = estimate_costs(24, platform='450K', config=test_config)
        cost_epic = estimate_costs(24, platform='EPIC', config=test_config)
        cost_wgbs = estimate_costs(24, platform='WGBS', config=test_config)
        
        # WGBS should be most expensive
        assert cost_wgbs['total'] > cost_epic['total']
        assert cost_epic['total'] > cost_450k['total']
...
class TestTimelineEstimation:
    ...
    def test_sample_size_scaling(self, test_config):
        ...
        assert timeline_large['total_duration_days'] > \
    ...
    def test_optional_phases(self, test_config):
        ...
        timeline_required = estimate_timeline(
            24,
            include_optional_phases=False,
            config=test_config
        )
        
        assert len(timeline_all['phases']) > len(timeline_required['phases'])
        assert timeline_all['total_duration_days'] > \
    ...
    def test_platform_differences(self, test_config):
        ...
        # WGBS takes longer
        assert timeline_wgbs['total_duration_days'] > \
               timeline_epic['total_duration_days']
...
class TestBatchPlanning:
    ...
    def test_unbalanced_detection(self):
        ...
        if not plan['balanced']:
            assert any('unbalanced' in rec.lower() 
...
class TestRiskAssessment:
    ...
    def test_power_risks(self, test_config):
        ...
        risks_low = assess_study_risks(
            n_samples=24,
            power=0.65,
            config=test_config
        )
        
        risks_high = assess_study_risks(
            n_samples=24,
            power=0.85,
            config=test_config
        )
    ...
    def test_budget_risks(self, test_config):
        ...
        budget_risks = [r for r in risks['risks'] 
    ...
    def test_budget_severity_levels(self, test_config):
        ...
        minor_budget = [r for r in risks_minor['risks'] 
                       if r['category'] == 'Budget'][0]
        major_budget = [r for r in risks_major['risks'] 
                       if r['category'] == 'Budget'][0]
    ...
    def test_sample_size_risks(self, test_config):
        ...
        size_risks = [r for r in risks['risks'] 
                     if r['category'] == 'Sample Size']
    ...
    def test_batch_risks(self, test_config):
        ...
        batch_risks = [r for r in risks['risks'] 
                      if 'batch' in r['risk'].lower()]
    ...
    def test_platform_risks(self, test_config):
        ...
        risks = assess_study_risks(
            n_samples=24,
            power=0.80,
            platform='27K',  # Discontinued
            config=test_config
        )
        
        platform_risks = [r for r in risks['risks'] 
...
class TestComprehensiveStudyPlan:
    ...
    def test_output_files_created(self, test_config, temp_output_dir):
        ...
        assert (output_dir / 'study_plan_report.txt').exists()
        assert (output_dir / 'study_plan.xlsx').exists()
        assert (output_dir / 'power_curves.png').exists()
        assert (output_dir / 'sample_size_heatmap.png').exists()
